...
| 卡牌查询  | 稀有度/属性/技能/抽卡台词/综合力/特训消耗/实装与进档案时间/卡面（通常+特训后），支持中文/日文/英文名与卡号 |
| 乐曲查询  | 作者/作词/作曲/编曲/演唱者/全部难度谱面（EZ~AP 等级与 Note 数）/曲封  |
| 活动查询  | 当前进行中活动、最近 N 期列表（活动图/奖励发放与关闭时间/排名奖励档位） |
| 卡池查询  | 当前卡池（横幅图/概率/心愿选择/角色自选/介绍文案）与 UP 卡池 |
...
| `pj卡 <卡名/角色名/卡号>`             | 卡牌查询            |
| `pj曲 <曲名>`                    | 乐曲与全难度谱面        |
| `pj活动 [编号]`                    | 当前进行中活动；加编号查看指定活动详情 |
| `pj活动列表 [数字]`                 | 最近 N 期活动列表（默认 8，带封面图）    |
| `pj卡池`                        | 当前卡池（概率/UP）     |
...
- 翻译表（zh-CN）来自 sekai-i18n，默认缓存 7 天；覆盖角色名/角色档案/组合名/卡牌名与技能/乐曲名/活动名
